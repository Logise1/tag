/* ==========================================
   ...
   Optimized for large player counts (up to 50)
   ========================================== */
...
const Network = {
    ...
    syncInterval: null,
    SYNC_RATE: 80,         // ms between syncs (lower = more updates but more bandwidth)
    rtdbListeners: [],
    _ready: false,
    _lastSentData: null,   // For delta compression
    ...
    _watchPeers() {
        ...
        const handler = ref.on('value', (snap) => {
            ...
            Object.entries(peers).forEach(([peerUid, peerId]) => {
                ...
                // Only connect if our uid is "lesser" to avoid duplicate connections
                if (uid < peerUid) {
    ...
    _setupConnection(conn, knownUid) {
        ...
        conn.on('close', () => {
            ...
            if (uid) delete this.uidPeerMap[uid];
    ...
    startSync(localPlayer) {
        ...
        // Adaptive sync: send full state less often, delta when idle
        this.syncInterval = setInterval(() => {
            if (!localPlayer) return;

            const data = localPlayer.toNetworkData();

            // Skip if player hasn't moved (reduce bandwidth for large rooms)
            if (this._lastSentData &&
                data.x === this._lastSentData.x &&
                data.y === this._lastSentData.y &&
                data.role === this._lastSentData.role &&
                data.points === this._lastSentData.points) {
                return; // Nothing changed, don't send
            }

            this._lastSentData = { ...data };
            this.broadcast({ type: 'state', uid: uid, data: data });
